"""Inference script for camera-trajectory prediction with CameraTransformer.

Loads a fine-tuned CameraTransformer checkpoint, runs inference on a set of
(reference image, text prompt) cases, converts the predicted camera pose
encoding into extrinsics/intrinsics, and visualizes the predicted trajectory.

Test cases are read from a JSON file (see assets/inference_cases/example_prompts.json).
...
# Default pinhole intrinsics used for visualization when GT intrinsics are unavailable.
...
def model_estimation(model, image_pth, prompt, result_saved_pth, unnorm_key):
    ...
    # Predict the camera-pose encoding sequence.
...
def parse_args():
    parser = argparse.ArgumentParser(description="CameraTransformer camera-trajectory inference.")
    ...
    parser.add_argument("--output-root", default="outputs",
                        help="Directory to write per-case outputs to.")
    parser.add_argument("--gather-root", default="gather_results",
                        help="Directory to write stitched result images to.")
    parser.add_argument("--checkpoint-filter", default="-epoch-03",
                        help="Substring used to select which checkpoints to run.")
    parser.add_argument("--unnorm-key", default="custom_finetuning",
                        help="Dataset key used for action de-normalization.")
    parser.add_argument("--action-model-type", default="DiT-B",
                        choices=["DiT-S", "DiT-B", "DiT-L"],
                        help="Must match the trained action-model size.")
...
def main():
    ...
    for checkpoint in tqdm(checkpoints):
        ...
        for case in test_cases:
            ...
            test_example_name = os.path.basename(image_pth).split(".")[0]

            output_dir_pth = os.path.join(args.output_root, ckpt_tag)
            os.makedirs(output_dir_pth, exist_ok=True)
            existed_file_length = len(os.listdir(output_dir_pth))
            test_example_name = f"{existed_file_length + 1}_{test_example_name}"
            ...
            # Rescale the [x, y, z] part while maintaining the rotation, then densify.
