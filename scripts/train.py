...
class TrainConfig:
    ...
    vla: VLAConfig = field(
        default_factory=VLAConfig.get_choice_class(VLARegistry.EXP_CAMERA_TRANSFORMER_OXE_MAGIC_SOUP_PLUS_MINUS.vla_id)
    )
    ...
    def __post_init__(self) -> None:
        ...
        print(f"self.vla.expected_world_size: {self.vla.expected_world_size}")
        print(f"overwatch.world_size(): {overwatch.world_size()}")
...
def train(cfg: TrainConfig) -> None:
    overwatch.info("CameraTransformer-VLA Training :: Warming Up")
    print(f"##### overwatch.local_rank(): {overwatch.local_rank()}")
    ...
    # Parameters (in millions): 7630.231 Total, 7630.231 Trainable
