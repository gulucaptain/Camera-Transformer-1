...
class CameraTransformer(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        print(f"### Here we load the vlm.projector")
        vlm.llm_backbone.load_state_dict(model_state_dict["llm_backbone"])
        print(f"### Here we load the vlm.llm_backbone")
        ...
        # Initialize CameraTransformer
        camera_transformer = CameraTransformer(vlm,
        ...
        inference_or_train = "inference" #FIXME
        # Load ActionModel from Checkpoint
        if "action_model" in model_state_dict:
            if inference_or_train == "train":
                new_action_model_state_dict = {}
                unneeded_keys = [
                    "net.history_embedder.linear.weight",
                    "net.x_embedder.linear.weight",
                    "net.final_layer.linear.weight",
                    "net.final_layer.linear.bias"
                ] #FIXME
                for key in model_state_dict["action_model"].keys():
                    if key not in unneeded_keys:
                        new_action_model_state_dict[key] = model_state_dict["action_model"][key]
                camera_transformer.action_model.load_state_dict(new_action_model_state_dict, strict=False) #FIXME
            elif inference_or_train == "inference":
                print(f"### Here we load the ACtion Model from the Finetuned checkpoints.")
                camera_transformer.action_model.load_state_dict(model_state_dict["action_model"], strict=True)
            
            print(f"### Here we load the camera_transformer.action_model")
            if "ema_diffusion" in model_state_dict and use_ema:
                camera_transformer.ema_diffusion.load_state_dict(model_state_dict["ema_diffusion"])
            elif use_ema:
                camera_transformer.ema_diffusion.load_state_dict(model_state_dict["action_model"])
        else:
            overwatch.warning("No ActionModel found in the pretrained checkpoint. Initializing a new one.")
        return camera_transformer
