...
declare -a arr=("path/to/camera-transformer-checkpoint")
# path/to/camera-transformer-large path/to/camera-transformer-small
...
do CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference.py --policy-model camera_transformer --ckpt-path ${ckpt_path} \
...
CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference.py --policy-model camera_transformer --ckpt-path ${ckpt_path} \
  --robot google_robot_static \
  --control-freq 3 --sim-freq 513 --max-episode-steps 80 \
  --env-name ${env_name} --scene-name ${scene_name} \
  --robot-init-x 0.35 0.35 1 --robot-init-y 0.21 0.21 1 --obj-variation-mode episode --obj-episode-range 0 60 \
